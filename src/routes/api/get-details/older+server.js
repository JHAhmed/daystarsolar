...
export async function POST({ request }) {
	const { fullName, consumerNumber, ebRegNumber, captcha } = await request.json();
	...
	try {
		...
		const supabaseData = await supabase 
			.from('reports')
			.insert([{ name: fullName, data: tempData }]);

		if (supabaseData.error) {
			console.error('Supabase insert error:', supabaseData.error);
			return new Response(JSON.stringify({ error: 'Failed to save report' }), { status: 500 });
		}

		console.log('Data saved to Supabase non-error:');
		...
		const response = new Response(JSON.stringify({ data: tempData }), { status: 500 });
		await browser.close();
		return response;
	}
}
